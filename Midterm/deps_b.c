...
void counterPusher(struct MessHall *hall)
{
    ...
    while (0 < limit) {
        ...
        if (0 < ready->P && 0 < ready->C && 0 < ready->D) {
            ...
            ready->C--;
            ready->D--;
            uint8_t isGradWaiting = 0 < hall->gradCount;
            if (isGradWaiting) {
                hall->gradCount--;
            } else {
                hall->undergradCount--;
...
void undergradMain(struct MessHall *hall, struct Actor *actor)
{
    ...
    for (size_t i = 1; i <= hall->args.L; ++i) {
        semWaitRepeat(&semC->m);
        hall->undergradCount++;
        printUnderArriving(actor->id, i, hall->undergradCount, cReady);
        sem_post(&semC->m);
        semWaitRepeat(&hall->underBarrier);

        printUnderWaiting(actor->id, i, hall->tablesEmpty);
        ...
        tables[tableId] = 1;
        hall->tablesEmpty--;
        printUnderSitting(actor->id, i, hall->tablesEmpty, tableId);
        //* Tables Consumer END
        sem_post(&semT->m);
        sem_post(&semT->empty);
        ...
        tables[tableId] = 0;
        if (i+1 != hall->args.L) {
            printUnderGoing(actor->id, i, hall->tablesEmpty, tableId);
        }
        //* Tables Producer END
        sem_post(&semT->m);
        sem_post(&semT->full);
    }
    printUnderFinished(actor->id, hall->args.L);
}
...
void gradMain(struct MessHall *hall, struct Actor *actor)
{
    ...
    for (size_t i = 1; i <= hall->args.L; ++i) {
        semWaitRepeat(&semC->m);
        hall->gradCount++;
        printGradArriving(actor->id, i, hall->gradCount, cReady);
        sem_post(&semC->m);
        semWaitRepeat(&hall->chooseBarrier);

        printGradWaiting(actor->id, i, hall->tablesEmpty);
        ...
        tables[tableId] = 1;
        hall->tablesEmpty--;
        printGradSitting(actor->id, i, hall->tablesEmpty, tableId);
        //* Tables Consumer END
        sem_post(&semT->m);
        sem_post(&semT->empty);
        ...
        tables[tableId] = 0;
        if (i+1 != hall->args.L) {
            printGradGoing(actor->id, i, hall->tablesEmpty, tableId);
        }
        //* Tables Producer END
        sem_post(&semT->m);
        sem_post(&semT->full);
    }
    printGradFinished(actor->id, hall->args.L);
}
...
{
    printPlateCounts(counts);
    sprintf(printBuf, "Undergrad %zu is going to the counter (round %zu) - # of students at counter: %zu and counter items %s\n", id, round, students, countStr);
    printStr(printBuf);
}

void printUnderWaiting(size_t id, size_t round, size_t tables)
{
    sprintf(printBuf, "Undergrad %zu got food and is going to get a table (round %zu) - # of empty tables: %zu\n", id, round, tables);
    printStr(printBuf);
}

void printUnderSitting(size_t id, size_t round, size_t tables, size_t tableId)
{
    sprintf(printBuf, "Undergrad %zu sat at the table %zu to eat (round %zu) - # of empty tables: %zu\n", id, tableId, round, tables);
    printStr(printBuf);
}

void printUnderGoing(size_t id, size_t round, size_t tables, size_t tableId)
{
    sprintf(printBuf, "Undergrad %zu left the table %zu to eat again (round %zu) - # of empty tables: %zu\n", id, tableId, round, tables);
    printStr(printBuf);
}

void printUnderFinished(size_t id, size_t rounds)
{
    sprintf(printBuf, "Undergrad %zu is done eating L=%zu times - going home - GOODBYE\n", id, rounds);
    printStr(printBuf);
}
...
{
    printPlateCounts(counts);
    sprintf(printBuf, "Graduate %zu is going to the counter (round %zu) - # of students at counter: %zu and counter items %s\n", id, round, students, countStr);
    printStr(printBuf);
}

void printGradWaiting(size_t id, size_t round, size_t tables)
{
    sprintf(printBuf, "Graduate %zu got food and is going to get a table (round %zu) - # of empty tables: %zu\n", id, round, tables);
    printStr(printBuf);
}

void printGradSitting(size_t id, size_t round, size_t tables, size_t tableId)
{
    sprintf(printBuf, "Graduate %zu sat at the table %zu to eat (round %zu) - # of empty tables: %zu\n", id, tableId, round, tables);
    printStr(printBuf);
}

void printGradGoing(size_t id, size_t round, size_t tables, size_t tableId)
{
    sprintf(printBuf, "Graduate %zu left the table %zu to eat again (round %zu) - # of empty tables: %zu\n", id, tableId, round, tables);
    printStr(printBuf);
}

void printGradFinished(size_t id, size_t rounds)
{
    sprintf(printBuf, "Graduate %zu is done eating L=%zu times - going home - GOODBYE\n", id, rounds);
    printStr(printBuf);
}