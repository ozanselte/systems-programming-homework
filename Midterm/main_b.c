...
int main(int argc, char *argv[])
{
    ...
    char optC;
    while (-1 != (optC = getopt(argc, argv, ":N:T:S:L:F:U:G:"))) {
        switch (optC) {
        case 'N':
        ...
        case 'L':
            ...
            break;
        case 'U':
            args->U = (size_t)strtoll(optarg, NULL, 10);
            break;
        case 'G':
            args->G = (size_t)strtoll(optarg, NULL, 10);
            break;
        case 'F':
...
void printUsage(const char *name)
{
    ...
    printStr(name);
    printStr(" -N <NUM> -U <NUM> -G <NUM> -T <NUM> -S <NUM> -L <NUM> -F <STRING>\n");
    printStr("All arguments are mandatory.\n");
    printStr("\t-N\t>2\tCount of the cooks\n");
    printStr("\t-U\t>= G\tCount of the students\n");
    printStr("\t-G\t>= 1\tCount of the students(U+G>=3)\n");
...
void initHallSemaphores(struct MessHall *hall)
{
    ...
    if (-1 == sem_init(&hall->underBarrier, 1, 0)) {
        errExit("initHallSemaphores, sem_init, &hall->underBarrier");
    }
    ...
    for (size_t i = 1; i <= hall->args.M; ++i) {
        if (0 == fork()) {
            ...
            if (i <= hall->args.U) {
                actor.id = i;
                actor.type = UNGR;
                undergradMain(hall, &actor);
            } else {
                actor.id = i - hall->args.U;
                actor.type = GRAD;
                gradMain(hall, &actor);
            }
            exitProcess(EXIT_SUCCESS);
...
void destroySemaphores(struct MessHall *hall)
{
    ...
    if (-1 == sem_destroy(&hall->underBarrier)) {
        errExit("destroySemaphores, sem_destroy, &hall->underBarrier");
    }
